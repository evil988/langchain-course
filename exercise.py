...
def implement_llama_4_model():
    ...
    """
    # Create ChatGroq with the exact Llama 4 model name and temperature 0
...
def implement_llama_3_3_model():
    ...
    """
    # Create ChatGroq with the exact Llama 3.3 model name and a slightly higher temperature
...
def implement_query_model(model, prompt):
    ...
    """
    try:
        # Build a chat-style message list and invoke the model
        messages = [{"role": "user", "content": prompt}]
        response = model.invoke(messages)

        # Return the textual content from the response
...
def implement_compare_models(prompt):
    ...
    # Return responses keyed by exact model names
    return {
        llama4.model: response4,
        llama33.model: response33,
    }
