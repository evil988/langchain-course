...
def implement_execute_tool_call(tool_call: Dict, available_tools: List[Tool]) -> str:
    ...
    tool = next((t for t in available_tools if getattr(t, "name", None) == tool_name), None)
    if tool is None:
        raise ValueError(f"Tool '{tool_name}' not found among available tools")
    
    # Extract arguments and map to the tool's expected single input
    ...
    if isinstance(args, dict):
        if "text" in args:
            ...
            # If only one arg is provided, pass its value directly
            input_data = next(iter(args.values()))
        else:
            # Fallback: pass the whole args dict
            input_data = args
    else:
        input_data = args
...
def implement_run_agent_with_tool_calling(model_with_tools: ChatOpenAI, 
    ...
    if implement_check_for_tool_calls(response):
        # 3. Execute the first tool call and return result
        first_tool_call = response.tool_calls[0]
        return implement_execute_tool_call(first_tool_call, available_tools)
    
    # 4. No tool calls: return model content
